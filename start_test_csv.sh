...
for csvfilefull in "${jmx_dir}"/*.csv; do
  csvfile="${csvfilefull##*/}"
  ...
  j=0
  for i in $(seq -f "%0${slavedigits}g" 0 $((slavesnum - 1))); do
    printf "Copy %s to %s on %s\n" "${i}.csv" "${csvfile}" "${slave_pods[j]}"
    kubectl -n "$namespace" cp "${jmx_dir}/${i}.csv" "${slave_pods[j]}":/tmp/
...
kubectl exec -ti -n "$namespace" "$master_pod" -- /jmeter/load_test "/tmp/${jmx_file}.jmx"
